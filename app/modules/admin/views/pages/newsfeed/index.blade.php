...
@section('content')
	...
	@endif
		...
		{{Form::open(array('route'=>'newsfeedDeleteAll','id'=>'form_data'))}}
		<div class="action-bar">
			<a class="btn btn-primary" href="{{URL::route('admin.newsfeed.create')}}"><span class="glyphicon glyphicon-plus"></span> Add New</a>
			...
			<table  width="100%" class="table tablesorter" data-page-size="13"  id="table" data-filter="#filter" data-filter-text-only="true">
	            ...
	            <tbody id="dulieu" >
		                ...
		                <tr>
		                    ...
		                    <td><a href="{{URL::route('admin.newsfeed.show',array('id'=>$item->id))}}" class="tieude">{{Str::words($item->title,15)}}</a></td>
		                    ...
		                    <td><a href="{{URL::route('admin.newsfeed.show',array('id'=>$item->id))}}" ><span class="glyphicon glyphicon-edit"></span> Edit</a> - <a href="javascript:avoid();" onclick="alertDelete({{$item->id}})" ><span class="glyphicon glyphicon-remove"></span> Remove </a></td>
		                </tr>
		                @endforeach
	            </tbody>
	            <tfoot>
...
@section('data_code')
	...
	<script type="text/javascript">
		...
		function alertDelete(id){
			var url = "{{URL::route('newsfeedDelete',array(':id'))}}";
		...
		$(document).ready(function(){
		    ...
		    $(".anhien-btn").on('click',function(){
		    	...
		    	if($(this).children('i').hasClass('on')){
		    		...
		    		$.ajax({
		    			url : '{{URL::route("admin_newsfeedVisible")}}',
		    			type: 'POST',
